...
def agreement_sum(domain_grads):
    ...
    # print("agr_mask.size() ", agr_mask.size())
    # Sum the components that have the same sign and zero those that do not
    # new_grads = torch.stack(domain_grads).sum(0)
    # new_grads *= agr_mask
    new_grads = torch.sum(domain_grads * agr_mask, 0)
    # print("domain_grads.size() ", domain_grads.size())
    # print("new_grads.size() ", new_grads.size())
    return new_grads
...
def agreement_rand(domain_grads):
    ...
    # new_grads = torch.stack(domain_grads).sum(0)
    # new_grads *= agr_mask
    # new_grads = domain_grads * agr_mask
    new_grads = torch.sum(domain_grads * agr_mask, 0)
    ...
    # print(agr_mask)
    # print(new_grads[agr_mask])
    # scale = new_grads[agr_mask.unsqueeze(0)].abs().mean()
...
def compute_agr_mask(domain_grads):
    ...
    grad_sign = torch.sign(domain_grads)
    # print("domain_grads.size() ", domain_grads.size())
