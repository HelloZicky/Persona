...
class gradient_avg_attention_l2(nn.Module):

    def __init__(self, group_num, grad_aggregation_layer):
        super(gradient_avg_attention_l2, self).__init__()
        ...
        for name, grad_tensor in gradient_dict.items():
            if name in self.grad_aggregation_layer:
                ...
                if torch.sum(mean_grad_similarity_matrix_sign) >= 0:
                    ...
                    similarity_gradient_dict[name] = torch.randn(gradient_dict[name][0].size()).to(gradient_dict[name][0].device) / 100
