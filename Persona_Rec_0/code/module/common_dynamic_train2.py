...
class HyperNetwork_FC_grad_gru_c(nn.Module):
    ...
    def forward(self, x, z, sample_num=1024, trigger_seq_length=30, pretrain_model=None, grad_norm=1,
                ...
                return_grad=False, z_center=None):
        units = self.units
        # z = z.view(sample_num, -1)
...
class HyperNetwork_FC_grad_gru_center_dynamic(nn.Module):
    ...
    def forward(self, x, z, sample_num=1024, trigger_seq_length=30, pretrain_model=None, grad_norm=1,
                # return_grad=False, z_center=None, return_mini_grad=True):
                return_grad=False, center_z=None, dynamic_partition=False):
        # if return_mini_grad:
        # if return_grad:
        if dynamic_partition:
            ...
            for i in range(1, len(units)):
                ...
                # weight = torch.matmul(z, self.w1[index]) + self.b1[index]
                weight_mini = torch.clip(torch.matmul(z, self.w1[index]) + self.b1[index], min=-grad_norm_, max=grad_norm_)
                # weight_center = torch.matmul(center_z[:, :, index * self.id_dimension: (index + 1) * self.id_dimension],
                #                              self.w1[index]) + self.b1[index]
                # print(center_z.size())
                ...
                bias_center = torch.matmul(center_z[:, index * id_dimension: (index + 1) * id_dimension], self.w2[index]) + self.b2[index]
