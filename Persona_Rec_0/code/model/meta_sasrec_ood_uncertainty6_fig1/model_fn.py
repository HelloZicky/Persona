...
@model("meta_sasrec_ood_uncertainty6_fig1", MetaType.ModelBuilder)
class SasRec(nn.Module):
    ...
    def forward(self, features, pred=False, uncertainty_threshold=0.99, mis_rec_threshold=0.2, stage=0, use_uncertainty_net=False):
        ...
        if not pred:
            ...
            # if use_uncertainty_net:
                ...
                for i in range(sample_times):
                    ...
                    # mis_rec_pred = self.stage2_pred_decoder(torch.cat([user_embedding, z_], dim=-1))
                    mis_rec_pred_list.append(mis_rec_pred)

                # uncertainty = torch.sum(torch.var(torch.Tensor(mis_rec_pred_list), dim=-1), dim=-1)
                mis_rec_pred = torch.stack(mis_rec_pred_list, dim=1)
                mis_rec_pred_mean = torch.mean(mis_rec_pred, dim=1)
                # uncertainty = torch.sum(torch.var(mis_rec_pred, dim=-1), dim=-1)
                ...
                # mis_rec_pred = self.stage3_mis_recommendation_layer2(torch.cat([mis_rec_pred, uncertainty], dim=1))
            ...
            # if use_uncertainty_net:
                ...
                # print(next_item_embedding_pred.size())
                # print(next_item_embedding_pred.squeeze(1).size())
                # print(target_embed.size())
                mse_list = torch.mean(criterion_list(next_item_embedding_pred.squeeze(1),
                                                     target_embed), dim=1)
                # print(mse_list.size())
