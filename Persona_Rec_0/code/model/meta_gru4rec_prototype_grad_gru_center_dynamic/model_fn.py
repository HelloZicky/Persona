...
@model("meta_gru4rec_prototype_grad_gru_center_dynamic", MetaType.ModelBuilder)
class GRU4Rec(nn.Module):
    def __init__(self, model_conf):
        ...
        # self._meta_classifier_param_list = common.HyperNetwork_FC_grad_gru_center(
    ...
    def forward(self, features, center_z=None, fig1=False, pretrain_model=None, return_grad=False, grad_norm_=0.5,
                dynamic_partition=False, stage=0):
        # Encode target item
        ...
        if dynamic_partition:
            user_embedding, grad_list, mini_grad_list = self._meta_classifier_param_list(user_state, hist_embed,
                                                                                         user_state.size()[0],
                                                                                         seq_length,
                                                                                         pretrain_model=pretrain_model,
                                                                                         return_grad=return_grad,
                                                                                         center_z=center_z, grad_norm_=grad_norm_,
                                                                                         dynamic_partition=dynamic_partition,
                                                                                         stage=stage
                                                                                         )
            return torch.sum(user_embedding * target_embed, dim=1, keepdim=True), grad_list, mini_grad_list
        else:
            if return_grad:
        ...
        #                                                       return_grad=return_grad,
        #                                                                  center_z=center_z,
        #                                                                  return_mini_grad=return_mini_grad
        #                                                       )
        #     if fig1:
        ...
        #                                                                       return_mini_grad=return_mini_grad
        #                                                                       )
        #         return torch.sum(user_embedding * target_embed, dim=1, keepdim=True), torch.sum(
        #             user_embedding1 * target_embed, dim=1, keepdim=True)
        #
        #     return torch.sum(user_embedding * target_embed, dim=1, keepdim=True), grad_list, mini_grad_list
        # else:
        #     user_embedding = self._meta_classifier_param_list(user_state, hist_embed,
