...
for dataset in DATASET_LIST:
    ...
    for model in MODEL_LIST:
        ...
        for type in TYPE_LIST:
            ...
            plt.plot(mis_rec[0], mis_rec[1], color="b", alpha=0.5)
            plt.hlines(np.mean(mis_rec[1]), 1, group_num - start_index, linestyle='-.', colors="grey")
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_mrs.png'.format("fig1", type)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_mrs.pdf'.format("fig1", type)), format='pdf', bbox_inches='tight')
            plt.clf()
            plt.cla()
