...
for dataset in DATASET_LIST:
    ...
    for model in MODEL_LIST:
        ...
        for type in TYPE_LIST:
            ...
            y_max = max(auc_user_list + auc_user_trigger_list)
            y_min = min(auc_user_list + auc_user_trigger_list)
            ...
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}2.png'.format("fig1", type)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}2.pdf'.format("fig1", type)), format='pdf', bbox_inches='tight')
            ...
            # plt.ylabel("AUC", fontsize=14)
            plt.tick_params(labelsize=13)
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_prifit2.png'.format("fig1", type)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_prifit2.pdf'.format("fig1", type)), format='pdf', bbox_inches='tight')
            ...
            # plt.ylabel("AUC", fontsize=16)
            plt.tick_params(labelsize=13)
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_overall2.png'.format("fig1", type)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_overall2.pdf'.format("fig1", type)), format='pdf', bbox_inches='tight')

            plt.clf()
            plt.cla()

            f.set_figheight(1)
            y_max = max(mis_rec_list)
            y_min = min(mis_rec_list)
            plt.ylim((y_min - 0.008) * 1000 // 10 / 100, (y_max + 0.01) * 1000 // 10 / 100)
            list_a = np.arange((y_min - 0.008) * 1000 // 10 / 100, (y_max + 0.01) * 1000 // 10 / 100, 0.01)
            plt.yticks(list_a)
            plt.xticks([])
            ...
            plt.plot(mis_rec[0], mis_rec[1], color="b", alpha=0.5)
            plt.hlines(np.mean(mis_rec[1]), 1, group_num - start_index, linestyle='-.', colors="grey")
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_mrs2.png'.format("fig1", type)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}'.format(type), '{}_{}_mrs2.pdf'.format("fig1", type)), format='pdf', bbox_inches='tight')
            plt.clf()
            plt.cla()
