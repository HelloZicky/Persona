...
TYPE_LIST = ["meta_ood_ocsvm_apg", "meta_ood_lof_apg", "meta_random_apg", "meta_ood_apg", "meta_ood_uncertainty5_apg"]
...
TYPE_LIST_reverse = ["meta_random_apg", "meta_ood_apg", "meta_ood_ocsvm_apg", "meta_ood_lof_apg", "meta_ood_uncertainty5_apg", "meta_ood_uncertainty5_2_apg"]
...
fig_file = os.path.join(ROOT_FOLDER, "request_curve_apg.png")
...
for dataset in DATASET_LIST:
    ...
    for model in MODEL_LIST:
        ...
        # for metric_name, metric_dict, ylabel in zip(["auc", "auc_user", "ndcg5", "ndcg10",
        #                                      "ndcg20", "hr5", "hr10", "hr20"],
        #                                     [auc_dict, auc_user_dict, logloss_dict, ndcg5_dict, ndcg10_dict,
        #                                      ndcg20_dict, hr5_dict, hr10_dict, hr20_dict],
        #                                     ["AUC", "UAUC", "NDCG@5", "NDCG@10",
        #                                      "NDCG@20", "HR@5", "HR@10", "HR@20"],):
        # for metric_name, metric_dict in zip(["auc", "auc_user", "ndcg5", "ndcg10",
        #                                      "ndcg20", "hr5", "hr10", "hr20"],
        #                                     [auc_dict, auc_user_dict, ndcg5_dict, ndcg10_dict,
        #                                      ndcg20_dict, hr5_dict, hr10_dict, hr20_dict]):
            ...
            metric_list = []
            rate_list = []
            for type, color, legend_name, shape in zip(TYPE_LIST + ["meta_apg"], ["green", "orange", "red", "blue", "black", "skyblue"],
            ...
            figs_apg_folder = os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), "figs_apg")
            if not os.path.exists(figs_apg_folder):
                os.makedirs(figs_apg_folder)
            # plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}_apg.png'.format(metric_name)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(figs_apg_folder, '{}.png'.format(metric_name)), format='png', bbox_inches='tight')
            # plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}_apg.pdf'.format(metric_name)), format='pdf', bbox_inches='tight')
            plt.savefig(os.path.join(figs_apg_folder, '{}.pdf'.format(metric_name)), format='pdf', bbox_inches='tight')
            # plt.sca(ax)
            # plt.close()
            plt.clf()
            plt.cla()
