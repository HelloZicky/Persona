...
TYPE_LIST = ["meta_ood_ocsvm_apg", "meta_ood_lof_apg", "meta_random_apg", "meta_ood_uncertainty5_apg"]
# TYPE_LIST = ["meta_ood_ocsvm", "meta_ood_lof", "meta_random", "meta_ood_uncertainty5"]
# TYPE_LIST__ = ["meta_random_apg", "meta_ood_uncertainty5_apg", "meta_ood_ocsvm_apg", "meta_ood_lof_apg", ]
...
for dataset in DATASET_LIST:
    ...
    for model in MODEL_LIST:
        ...
        for type in TYPE_LIST + ["meta_apg"]:
            print("-" * 50)
            print("type ", type)
            # if type in ["meta_random", "meta_ood", "meta_ood_uncertainty6"]:
            if type in ["meta_random_apg", "meta_ood_apg", "meta_ood_uncertainty_apg", "meta_ood_uncertainty5_apg", "meta_ood_uncertainty6_apg", "meta_ood_ocsvm_apg", "meta_ood_lof_apg"]:
            ...
            if type in ["meta_random_apg", "meta_ood_apg", "meta_ood_uncertainty_apg", "meta_ood_uncertainty5_apg", "meta_ood_uncertainty6_apg", "meta_ood_lof_apg", "meta_ood_ocsvm_apg"]:
        ...
        #                                      ndcg20_dict, hr5_dict, hr10_dict, hr20_dict]):
            ...
            rate_list = []
            temp_dict = {}
            for type, color, legend_name, shape in zip(TYPE_LIST + ["meta_apg"], ["green", "orange", "red", "black", "skyblue"],
                                                   # ["DUET (OC-SVM)", "DUET (LOF)", "DUET (Random)", "MR-DUET (w/o DM)", "MR-DUET (DM5)", "DUET"],
                                                   # ["DUET (OC-SVM)", "DUET (LOF)", "DUET (Random)", "MR-DUET", "DUET"],
                                                   ["DUET (OC-SVM)", "DUET (LOF)", "DUET (Random)", "MR-DUET", "DUET"],
                                                   ["*", "o", "s", "x", ".", "_", "_", "_"]):
            # for type, color, legend_name, shape in zip(TYPE_LIST__ + ["meta_apg"], ["red", "black", "green", "orange", "skyblue"],
            # for type, color, legend_name, shape in zip(TYPE_LIST__ + ["meta_apg"], ["red", "black", "green", "orange", "skyblue"],
            #                                            # ["DUET (OC-SVM)", "DUET (LOF)", "DUET (Random)", "MR-DUET (w/o DM)", "MR-DUET (DM5)", "DUET"],
            #                                            ["DUET (Random)", "MR-DUET", "DUET (OC-SVM)", "DUET (LOF)", "DUET"],
            #                                            ["_", "_", "*", "o", "s", "x", ".", "_"]):
                # TYPE_LIST = ["meta_random", "meta_ood", "meta_ood_ocsvm", "meta_ood_lof"]
                ...
                if type in ["meta_random_apg", "meta_ood_apg", "meta_ood_uncertainty_apg", "meta_ood_uncertainty5_apg", "meta_ood_uncertainty6_apg", "meta_ood_ocsvm_apg", "meta_ood_lof_apg"]:
                    ...
                    if type in ["meta_ood_ocsvm_apg", "meta_ood_lof_apg"]:
                        # if type in ["meta"]:
                        # print()
                        # rate_dict[type][min_index] = temp_dict[min_index]
                        # # rate_dict[type][max_index] = temp_dict[max_index]
                        # rate_dict[type][-1] = temp_dict[-1]
                        print("*** type", type)
                        # temp_dict = rate_dict[type]
                        temp_dict = metric_dict[type]
                        print("*** temp_dict", temp_dict)
                    elif type not in ["meta_ood_ocsvm_apg", "meta_ood_lof_apg"]:
                        # temp_dict = rate_dict[type]
                        print("***", type)
                        print("***", metric_dict[type])
                        metric_dict[type][min_index] = temp_dict[min_index]
                        metric_dict[type][-1] = temp_dict[-1]
                        print("***", metric_dict[type])
            ...
            metric_list = [i for i in metric_list if i != (float('NaN')) and i != (float('Inf'))]
            ...
            plt.yticks(np.arange(round((y_min), 3), round((y_max), 3), 0.002))
            ...
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}_main_apg.png'.format(metric_name)), format='png', bbox_inches='tight')
            plt.savefig(os.path.join(ROOT_FOLDER, "{}_{}".format(dataset, model), '{}_main_apg.pdf'.format(metric_name)), format='pdf', bbox_inches='tight')
            # plt.sca(ax)
            # plt.close()
            plt.clf()
            plt.cla()
