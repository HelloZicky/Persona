...
#CHECKPOINT_PATH=../checkpoint/NIPS2023/${dataset}_${model}/meta_grad_gru_c
CHECKPOINT_PATH=../checkpoint/NIPS2023/${dataset}_${model}/meta_grad_gru
#pretrain_model_path=../checkpoint/NIPS2023/${dataset}_${model}/base_group_finetune_group_${}/
...
#USER_DEFINED_ARGS="--model=meta_${model}_prototype_grad_gru_c --num_loading_workers=1 --arch_config=${ARCH_CONFIG_CONTENT} --learning_rate=${LEARNING_RATE} \
...
T_PATH} --arch_config=${ARCH_CONF_FILE} --pretrain_model_path=${pretrain_model_path} --pretrain --grad_norm=${grad_norm}"
