...
def smooth_xy(lx, ly):
    ...
    x = np.array(lx)
    y = np.array(ly)
    x_smooth = np.linspace(min(x), max(x), 25)
    ...
    for model in MODEL_LIST:
        ...
        #                                      ndcg20_dict, hr5_dict, hr10_dict, hr20_dict]):
            ...
            for type, color, legend_name, shape in zip(TYPE_LIST + ["meta"], ["green", "orange", "red", "blue", "black", "skyblue"],
                ...
                if type in ["meta_ood_ocsvm", "meta_ood_lof"]:
                    continue
                elif type in ["meta_random", "meta_ood", "meta_ood_uncertainty", "meta_ood_uncertainty5", "meta_ood_uncertainty6", "meta_ood_ocsvm", "meta_ood_lof"]:
                    # print(1)
                    # print(type)
                    # print(metric_name)
                    # print(rate_dict[type])
                    ...
                    plt.plot(rate_dict_temp, metric_dict_temp, color=color, label=legend_name)
