...
CHECKPOINT_PATH=../checkpoint/NIPS2023/${dataset}_${model}/meta_grad_gru_center_group_${group_num}_dynamic
...
#pretrain_meta_model_path=../checkpoint/NIPS2023/${dataset}_${model}/meta_grad_gru_c/best_auc.pkl
######################################################################
...
USER_DEFINED_ARGS="--model=meta_${model}_prototype_grad_gru_center_dynamic --num_loading_workers=1 --arch_config=${ARCH_CONFIG_CONTENT} --learning_rate=${LEARNING_RATE} \
--max_gradient_norm=${GRADIENT_CLIP} --batch_size=${BATCH_SIZE} --snapshot=${SNAPSHOT} --max_steps=${ITERATION} --checkpoint_dir=${CHECKPOIN\
T_PATH} --arch_config=${ARCH_CONF_FILE} --pretrain_base_model_path=${pretrain_base_model_path}
...
test_file_overall="${dataset}/test.txt"
...
python ../main_new/multi_metric_meta_grad_group_center_dynamic_train2.py \
--dataset=${data} \
--cluster_center_file=${cluster_center_file} \
--test_file_overall=${test_file_overall} \
${USER_DEFINED_ARGS}
