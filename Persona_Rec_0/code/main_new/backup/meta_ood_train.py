...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, epoch=0):
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        if step % log_every == 0:
            logger.info(
                "train_epoch={}, step={}, auc={:5f}, ood_auc={:5f}, ndcg={:5f}, speed={:2f} steps/s".format(
                    train_epoch, step, auc, ood_auc, ndcg, log_every / timer.tick(False)
                )
            )
    ...
    print("train_epoch={}, train_step={}, auc={:5f}, ood_auc={:5f}, ndcg={:5f}".format(
        train_epoch, train_step, auc, ood_auc, ndcg
    ))
...
def train(train_dataset, model_obj, device, args, pred_dataloader, model_load_name_set):
