...
def load_model(args):
    ...
    base_ckpt_path = os.path.join(ckpt_path)
    ...
    # base_model_load = torch.load(base_ckpt_path)
    base_model_load = torch.load(base_ckpt_path, map_location=torch.device('cpu'))
    base_model_load_name_set = set()
    for name, parms in base_model_load.named_parameters():
        base_model_load_name_set.add(name)
    print(base_model_load_name_set)
    ...
    return base_model_load, base_model_load_name_set
...
def train(train_dataloader_list, predict_dataloader_list, base_model_obj, device, args, pretrain_model_param_dict=None,
    ...
    if args.train_mode == "long_tail":
        args.save_dir = os.path.join(args.checkpoint_dir, "base_group_finetune_long_tail_{}".format(args.group_num))
    elif args.train_mode == "meta_grad":
        args.save_dir = os.path.join(args.checkpoint_dir, "base_group_finetune_grad_{}".format(args.group_num))
    elif args.train_mode == "meta_grad_gru":
        args.save_dir = os.path.join(args.checkpoint_dir, "base_group_finetune_grad_gru_{}".format(args.group_num))
    ...
    model_obj_backup = deepcopy(base_model_obj)
    ...
    for group_index in range(args.group_num):
        ...
        best_auc_ckpt_path_dict[group_index] = os.path.join(args.save_dir,
...
def main_worker(_):
    ...
    base_model_obj, base_model_load_name_set = load_model(args)
    ...
    for group_index in range(args.group_num):
        # train_file = os.path.join(args.dataset, "domain_label_grad_random_5", args.base_model, str(args.group_num), "train_{}.txt".format(group_index))
        # test_file = os.path.join(args.dataset, "domain_label_grad_random_5", args.base_model, str(args.group_num), "test_{}.txt".format(group_index))
        # train_file = os.path.join(args.dataset, "domain_label_grad", args.model, str(args.group_num), "train_{}.txt".format(group_index))
        # test_file = os.path.join(args.dataset, "domain_label_grad", args.model, str(args.group_num), "test_{}.txt".format(group_index))
        # if args.long_tail:
        if args.train_mode == "long_tail":
