...
def predict_uncertainty(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, epoch=0, train_ood_threshold=0.5):
    # Load pretrained models
...
def train(positive_train_dataset, train_dataset, model_uncertainty_obj, model_obj, device, args,
    ...
    ### stage1: Next-item Prediction model
    ...
    for epoch in range(1, args.max_epoch_uncertainty + 1):
        for step, batch_data in enumerate(positive_train_dataset, 1):
            ...
            # pred_embed = model_uncertainty_obj({
            ...
            # loss_next_item
            # loss_regularzation = l2_regularisation(model_uncertainty_obj.posterior) + \
            #                        l2_regularisation(model_uncertainty_obj.prior) + \
            #                        l2_regularisation(model_uncertainty_obj.fcomb.layers)

            # loss = loss_next_item + 1e-5 * loss_uncertainty_reg
            # loss = loss_cvae + 1e-5 * loss_regularzation
            loss = loss_cvae
        ...
        pred_mse, pred_uncertainty, _ood_threshold = predict_uncertainty(
    ...
    ### stage2: RecSys model and Mis-recommendation model
