...
def train(positive_train_dataset, train_dataset, model_uncertainty_obj, model_obj, device, args, positive_pred_dataset, pred_dataloader, model_load_name_set=None):
    ...
    for epoch in range(1, args.max_epoch + 1):
        for step, batch_data in enumerate(positive_train_dataset, 1):
            ...
            loss_next_item
            loss_next_item_reg = l2_regularisation(model_uncertainty_obj.posterior) + \
...
def main_worker(_):
    ...
    # Construct model
    model_uncertainty_obj = model_meta.model_builder(model_conf=model_uncertainty_conf)  # type: torch.nn.module
    ...
    train(
        ...
        args=args,
        # bucket=bucket,
        positive_pred_dataset=positive_pred_dataloader,
