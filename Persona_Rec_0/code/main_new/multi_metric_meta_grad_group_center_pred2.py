...
def parse_args():
    ...
    parser.add_argument("--train_mode", type=str, default=None, help="base64-encoded model configs")
    parser.add_argument("--dataset_folder", type=str, default=None, help="base64-encoded model configs")
...
def load_model(args, model_obj_meta, model_obj_base):
    ...
    base_model_load_dict = {}
    base_model_load_name_set_dict = {}
    for group_index in range(args.group_num):
        base_ckpt_path = os.path.join(args.pretrain_base_model_path, "best_auc_{}.pkl".format(group_index))
        ...
        base_model_load_dict[group_index] = base_model_load
        base_model_load_name_set_dict[group_index] = base_model_load_name_set
        print(base_model_load_name_set)

    meta_model_load_dict = {}
    meta_model_load_name_set_dict = {}
    for group_index in range(args.group_num):
        meta_ckpt_path = os.path.join(args.save_dir, "best_auc_{}.pkl".format(group_index))
        ...
        meta_model_load_dict[group_index] = meta_model_load
        meta_model_load_name_set_dict[group_index] = meta_model_load_name_set
    ...
    # return base_model_load_dict, base_model_load_name_set_dict, meta_model_load, meta_model_load_name_set
    return base_model_load_dict, base_model_load_name_set_dict, meta_model_load_dict, meta_model_load_name_set_dict
...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None,
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        # }, pretrain_model=pretrain_model_param_dict, grad_norm_=args.grad_norm_)
...
def train(train_dataloader_list, predict_dataloader_list, base_model_obj, meta_model_obj, device, args, pretrain_model_param_dict=None,
    ...
    pretrain_model_param_dict = {}
    for group_index in range(args.group_num):
        pretrain_model_param_dict[group_index] = base_model_obj[group_index].state_dict()
    ...
    # if args.train_mode == "long_tail":
    #     args.save_dir = os.path.join(args.checkpoint_dir, "meta_long_tail_center_{}_clip_{}".format(args.group_num, args.grad_norm))
    # elif args.train_mode == "meta_grad":
    #     args.save_dir = os.path.join(args.checkpoint_dir, "meta_grad_center_{}_clip_{}".format(args.group_num, args.grad_norm))
    # elif args.train_mode == "meta_grad_gru":
    #     # args.save_dir = os.path.join(args.checkpoint_dir, "base_group_finetune_group_{}".format(args.group_num))
    #     args.save_dir = os.path.join(args.checkpoint_dir, "meta_grad_gru_center_{}_clip_{}".format(args.group_num, args.grad_norm))

    # if args.train_mode == "long_tail":
    #     dataset_folder = os.path.join(args.dataset, "long_tail", "group_{}".format(str(args.group_num)))
    # elif args.train_mode == "meta_grad":
    #     dataset_folder = os.path.join(args.dataset, "domain_label_grad_random_{}_final".format(random_seed), args.model,
    #                                   str(args.group_num))
    # elif args.train_mode == "meta_grad_gru":
    #     dataset_folder = os.path.join(args.dataset, "domain_label_grad_gru_random_{}_final".format(random_seed),
    #                                   args.model, str(args.group_num))

    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)

    model_dict = meta_model_obj
    # for group_index in range(args.group_num):
        # model_dict[group_index] = deepcopy(model_obj_backup)
        # model_dict[group_index] = deepcopy(meta_model_obj)
        # model_dict[group_index] = meta_model_obj
        ...
        # best_auc_ckpt_path_dict[group_index] = os.path.join(args.save_dir, "best_auc_{}".format(group_index) + ".pkl")
    y_dict_overall = {}
    pred_dict_overall = {}
    buffer_dict_overall = {}
    ...
    for group_index in range(args.group_num):
        ...
        #         for key, value in batch_data.items()
        #         if key not in {consts.FIELD_USER_ID, consts.FIELD_LABEL}
        #     # }, pretrain_model=pretrain_model_param_dict[group_index], grad_norm_=args.grad_norm_)
        #     }, pretrain_model=pretrain_model_param_dict[group_index], grad_norm=args.grad_norm)
        #
        #     # if step == 1:
        #     #     # from thop import profile
        ...
        if predict_dataset_length > 0:
            ...
            predict(
                ...
                train_step=0,
                # writer=writer,
                pretrain_model_param_dict=pretrain_model_param_dict[group_index],
            ...
            y_list_group.extend(y_list)
            pred_list_group.extend(pred_list)
            buffer_group.extend(buffer)
        ...
        epoch = 0
        step = 0
        group_auc, _, _, _ = new_metrics.calculate_overall_auc(np.array(pred_list_group),
                                                                 np.array(y_list_group))
        user_auc = new_metrics.calculate_user_auc(buffer_group)
        group_logloss = new_metrics.calculate_overall_logloss(np.array(pred_list_group),
                                                                np.array(y_list_group))
        # user_ndcg, user_hr = new_metrics.calculate_user_ndcg_hr(10, np.array(pred_list), np.array(y_list))
        user_ndcg5, user_hr5 = new_metrics.calculate_user_ndcg_hr(5, buffer_group)
        user_ndcg10, user_hr10 = new_metrics.calculate_user_ndcg_hr(10, buffer_group)
        user_ndcg20, user_hr20 = new_metrics.calculate_user_ndcg_hr(20, buffer_group)
        # ndcg = 0

        print("epoch={}, step={}, group_auc={:5f}, user_auc={:5f}, group_logloss={:5f}, "
              "user_ndcg5={:5f}, user_hr5={:5f}, user_ndcg10={:5f}, user_hr10={:5f}, user_ndcg20={:5f}, user_hr20={:5f}".
              format(epoch, 1, group_auc, user_auc, group_logloss,
                     user_ndcg5, user_hr5, user_ndcg10, user_hr10, user_ndcg20, user_hr20))
        # with open(os.path.join(args.checkpoint_dir, "log_ood.txt"), "a") as writer:
        # with open(os.path.join(args.save_dir, "log_ood.txt"), "a") as writer:
        # with open(os.path.join(args.save_dir, "group_{}_{}_test.txt".format(args.group_num, group_index)), "a") as writer:
        #     print("epoch={}, step={}, group_auc={:5f}, user_auc={:5f}, group_logloss={:5f}, "
        #           "user_ndcg5={:5f}, user_hr5={:5f}, user_ndcg10={:5f}, user_hr10={:5f}, user_ndcg20={:5f}, user_hr20={:5f}".
        #           format(epoch, 1, group_auc, user_auc, group_logloss,
        #                  user_ndcg5, user_hr5, user_ndcg10, user_hr10, user_ndcg20, user_hr20), file=writer)
        with open(os.path.join(args.save_dir, "group_{}_test.txt".format(args.group_num)), "a") as writer:
            print(group_auc, user_auc, group_logloss,
...
def main_worker(_):
    ...
    if args.train_mode == "long_tail":
        dataset_folder = os.path.join(args.dataset, "long_tail", "group_{}".format(str(args.group_num)))
        # args.pretrain_base_model_path = "base_group_finetune_group_{}".format(args.group_num)
        # args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_group_long_tail_{}".format(args.group_num))
        args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_long_tail_{}".format(args.group_num))
        args.pretrain_meta_model_path = os.path.join(args.pretrain_meta_model_path, "meta_grad_gru/best_auc.pkl")
    elif args.train_mode == "meta_grad":
        # dataset_folder = os.path.join(args.dataset, "domain_label_grad_random_{}_final".format(random_seed), args.model, str(args.group_num))
        dataset_folder = os.path.join(args.dataset, "domain_label_grad_random_{}_final".format(random_seed), args.base_model, str(args.group_num))
        # args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_group_no_gru_{}".format(args.group_num))
        args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_grad_{}".format(args.group_num))
        args.pretrain_meta_model_path = os.path.join(args.pretrain_meta_model_path, "meta_grad/best_auc.pkl")

    elif args.train_mode == "meta_grad_gru":
        dataset_folder = os.path.join(args.dataset, "domain_label_grad_gru_random_{}_final".format(random_seed), args.base_model, str(args.group_num))
        # args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_group_{}".format(args.group_num))
        args.pretrain_base_model_path = os.path.join(args.pretrain_base_model_path, "base_group_finetune_grad_gru_{}".format(args.group_num))
        args.pretrain_meta_model_path = os.path.join(args.pretrain_meta_model_path, "meta_grad_gru/best_auc.pkl")
    ...
    if args.train_mode == "long_tail":
        args.save_dir = os.path.join(args.checkpoint_dir,
                                     "meta_long_tail_center_{}_clip_{}".format(args.group_num, args.grad_norm))
    elif args.train_mode == "meta_grad":
        args.save_dir = os.path.join(args.checkpoint_dir,
                                     "meta_grad_center_{}_clip_{}".format(args.group_num, args.grad_norm))
    elif args.train_mode == "meta_grad_gru":
        # args.save_dir = os.path.join(args.checkpoint_dir, "base_group_finetune_group_{}".format(args.group_num))
        args.save_dir = os.path.join(args.checkpoint_dir,
                                     # "meta_grad_gru_center_{}_clip_{}".format(args.group_num, args.grad_norm))
                                     "meta_grad_gru_center_group_{}_clip_{}".format(args.group_num, args.grad_norm))
    ...
    #         os.path.join(args.dataset, "domain_label_grad_random_3", args.base_model, args.group_num, "train_{}.txt".format(i)))
    #     test_files.append(
    #         # os.path.join(args.dataset, "group_{}".format(args.group_num), "test", "test_{}.txt".format(i)))
    #         os.path.join(args.dataset, "domain_label_grad_random_3", args.base_model, args.group_num, "test_{}.txt".format(i)))

    # if train_mode == "meta_grad_gru":
    #     train_file = os.path.join(args.dataset, "domain_label_grad_random_0", args.base_model, str(args.group_num),
    #                               "train_{}.txt".format(group_index))
    #     test_file = os.path.join(args.dataset, "domain_label_grad_random_0", args.base_model, str(args.group_num),
    #                              "test_{}.txt".format(group_index))

    # if args.train_mode == "long_tail":
    #     dataset_folder = os.path.join(args.dataset, "long_tail", "group_{}".format(str(args.group_num)))
    # elif args.train_mode == "meta_grad":
    #     # dataset_folder = os.path.join(args.dataset, "domain_label_grad_", args.model, str(args.group_num))
    #     dataset_folder = os.path.join(args.dataset, "domain_label_grad_random_{}_final".format(random_seed), args.model, str(args.group_num))
    # elif args.train_mode == "meta_grad_gru":
    #     # dataset_folder = os.path.join(args.dataset, "domain_label_grad", args.model, str(args.group_num))
    #     dataset_folder = os.path.join(args.dataset, "domain_label_grad_gru_random_{}_final".format(random_seed), args.model, str(args.group_num))
    # else:
    #     raise FileNotFoundError

    for group_index in range(args.group_num):
        # train_file = os.path.join(args.dataset, "domain_label_grad_random_0", args.base_model, str(args.group_num), "train_{}.txt".format(group_index))
        # test_file = os.path.join(args.dataset, "domain_label_grad_random_0", args.base_model, str(args.group_num), "test_{}.txt".format(group_index))
        if args.train_mode == "long_tail":
            train_file = os.path.join(dataset_folder, "train", "train_{}.txt".format(group_index))
            test_file = os.path.join(dataset_folder, "test", "test_{}.txt".format(group_index))
        else:
            train_file = os.path.join(dataset_folder, "train_{}.txt".format(group_index))
