...
def parse_args():
    ...
    parser.add_argument("--model_dir", type=str, help="Path of the checkpoint path")
...
def get_sample_user_set(user_set, rate):
    # print(len(user_set))
    # print(len(user_set) * rate // 100)
    # print(int(round(len(user_set) * rate / 100, 0)))
    # print(int(len(user_set) * rate / 100, 0))
    sample_user_set = random.sample(user_set, int(round(len(user_set) * rate / 100, 0)))
    return sample_user_set
    ...
    with open(os.path.join(args.checkpoint_dir, "test.txt"), "w+") as writer:
        ...
        for rate in rate_list:
            if rate == 0:
                ...
                sample_user_set = get_sample_user_set(all_user_set, rate)
            sample_user_tensor = torch.Tensor(list(sample_user_set)).to(device)


            # if rate == 100:
            #     args.train_ood_threshold = torch.Tensor([max(ood_prob_list)])
            # else:
            #     args.train_ood_threshold = torch.Tensor([sorted(ood_prob_list)[len(ood_prob_list) * rate // 100]])
            pred_list = []
            pred_label_list = []
            y_list = []
            ood_pred_list = []
            ...
            for step, batch_data in tqdm(enumerate(predict_dataset, 1)):
                # logits, ood_logits, request_num, total_num = model_obj({
                logits, request_num, total_num = model_obj({
                    key: value.to(device)
                    for key, value in batch_data.items()
                    # if key not in {consts.FIELD_USER_ID, consts.FIELD_LABEL}
                    if key not in {consts.FIELD_LABEL}
                    # }, pred=True, train_ood_threshold=train_ood_threshold)
                # }, pred=True, train_ood_threshold=args.train_ood_threshold)
                }, pred=True, rate=rate, sample_user_tensor=sample_user_tensor)
                prob = torch.sigmoid(logits).detach().cpu().numpy()
                y = batch_data[consts.FIELD_LABEL].view(-1, 1)
            ...
            print("train_epoch={}, train_step={}, overall_auc={:5f}, user_auc={:5f}, overall_logloss={:5f}, "
                  ...
                  format(train_epoch, train_step, overall_auc, user_auc, overall_logloss,
                         ...
                         _request_num, _total_num, _threshold, rate))
...
def main_worker(_):
    ...
    if folder == "meta_ood":
        best_model = torch.load(os.path.join(args.checkpoint_dir, "best_auc_ood.pkl"))
            ...
            or folder == "meta_random_apg" or folder == "meta_ood_lof_apg" or folder == "meta_ood_ocsvm_apg":
        best_model = torch.load(os.path.join(args.checkpoint_dir, "best_auc.pkl"))
        # best_model = torch.load(os.path.join(args.model_dir, "best_auc.pkl"))
