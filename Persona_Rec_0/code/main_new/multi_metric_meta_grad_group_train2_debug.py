...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None):
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        }, pretrain_model=pretrain_model_param_dict)
...
def train(train_dataloader_list, predict_dataloader_list, model_obj, device, args):
    ...
    model_obj_backup = deepcopy(model_obj)
...
def main_worker(_):
    ...
    for i in range(args.group_num):
        train_files.append(
            ...
            os.path.join(args.dataset, "domain_label_grad", args.base_model, args.group_num, "train_{}.txt".format(i)))
        test_files.append(
            # os.path.join(args.dataset, "group_{}".format(args.group_num), "test", "test_{}.txt".format(i)))
            os.path.join(args.dataset, "domain_label_grad", args.base_model, args.group_num, "test_{}.txt".format(i)))
