...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, epoch=0):
    ...
    #     writer.write(buffer, [0, 1, 2])
    try:
        overall_auc = new_metrics.calculate_overall_auc(np.array(pred_list), np.array(y_list))
        user_auc = new_metrics.calculate_user_auc(buffer)
        ood_auc = new_metrics.calculate_overall_auc(np.array(ood_pred_list), np.array(ood_y_list))
    ...
    print("train_epoch={}, train_step={}, overall_auc={:5f}, user_auc={:5f}, overall_logloss={:5f}, "
          ...
          "ood_auc={:5f}, request_num={}, total_num={}".
          format(train_epoch, train_step, overall_auc, user_auc, overall_logloss,
                 ...
                 ood_auc, _request_num, _total_num))
    ...
    return overall_auc, user_auc, overall_logloss, user_ndcg5, user_hr5, user_ndcg10, user_hr10, user_ndcg20, user_hr20, ood_auc
...
def train(train_dataset, model_obj, device, args, pred_dataloader, model_load_name_set=None):
    ...
    for epoch in range(1, args.max_epoch + 1):
        for step, batch_data in enumerate(train_dataset, 1):
            ...
            try:
                auc = new_metrics.calculate_overall_auc(np.array(pred.detach().cpu()), np.array(y))
                ood_auc = new_metrics.calculate_overall_auc(np.array(ood_pred.detach().cpu()), np.array(ood_label))
            except Warning:
                print("-" * 50)
        ...
        pred_user_ndcg10, pred_user_hr10, pred_user_ndcg20, pred_user_hr20, pred_ood_auc = predict(
