...
def parse_args():
    ...
    parser.add_argument("--need_time_cost", type=bool, default=True, help="")
...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step,
            y_list_overall, pred_list_overall, writer=None, buffer_overall=None):
    ...
    # return user_auc, pred_list, y_list
...
def train(train_dataset, model_obj, device, args, pred_dataloader, y_list_overall, pred_list_overall, is_finetune, buffer_overall):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for step, batch_data in enumerate(train_dataset, 1):
            ...
            if args.need_time_cost:
                ...
                if epoch == 1:
                    end_time = time.time()
                    args.step_num += 1
                    args.sample_num += y.size()[0]
                    args.time_cost.append(end_time - start_time)
                    # time_cost_generate.append(generate_time)
        ...
        # if args.need_time_cost:
        #     if epoch == 1:
        #         time_cost_total = np.sum(time_cost)
        #         time_cost_per_step = time_cost_total / step_num
        #         time_cost_per_sample = time_cost_total / sample_num
        #         # time_cost_per_ = np.average(time_cost)
        #         # best_auc_ckpt_path = os.path.join(args.checkpoint_dir, "best_auc" + ".pkl")
        #         time_cost_path = os.path.join(args.checkpoint_dir, "time_cost_train" + ".txt")
        #         with open(time_cost_path, "w+") as writer:
        #             print(time_cost_total, time_cost_per_step, time_cost_per_sample, sample_num, sep="\t", file=writer)
...
def main_worker(_):
    ...
    args.checkpoint_dir = os.path.join(args.checkpoint_dir, "base_finetune_1_session")
    ...
    finetune_dataset_dir = os.path.join(args.dataset, "finetune_dataset_1_session")
    print(model_obj)
    ...
    args.time_cost_total = np.sum(args.time_cost)
    args.time_cost_per_step = args.time_cost_total / args.step_num
    args.time_cost_per_sample = args.time_cost_total / args.sample_num
    # time_cost_per_ = np.average(time_cost)
    # best_auc_ckpt_path = os.path.join(args.checkpoint_dir, "best_auc" + ".pkl")
    time_cost_path = os.path.join(args.checkpoint_dir, "time_cost_train" + ".txt")
    with open(time_cost_path, "w+") as writer:
        print(args.time_cost_total, args.time_cost_per_step, args.time_cost_per_sample, args.sample_num, sep="\t", file=writer)
