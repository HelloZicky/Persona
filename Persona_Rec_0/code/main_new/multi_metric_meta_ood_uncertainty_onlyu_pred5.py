...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, epoch=0, train_ood_threshold=0.5, best_model=None):
    ...
    for step, batch_data in tqdm(enumerate(predict_dataset, 1)):
        ...
        # }, pred=True, mis_rec_threshold=args.train_ood_threshold, use_uncertainty_net=False, stage=2)
        }, pred=True, mis_rec_threshold=args.train_ood_threshold, use_uncertainty_net=False, stage=3)
    ...
    sorted(rate_list)
    with open(os.path.join(args.save_dir, "test.txt"), "w+") as writer:
        ...
        for rate in rate_list:
            if rate == 100:
                ...
                args.train_ood_threshold = torch.Tensor([sorted(ood_prob_list, reverse=True)[int(round(len(ood_prob_list) * rate / 100, 0))]])
            pred_list = []
            pred_label_list = []
            y_list = []
            ood_pred_list = []
...
def main_worker(_):
    ...
    args.save_dir = args.checkpoint_dir.replace("uncertainty5", "uncertainty5_onlyu")
