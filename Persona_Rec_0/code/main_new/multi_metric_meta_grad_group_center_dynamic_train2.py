...
def predict_dynamic(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None,
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        for group_index in range(args.group_num):
            ...
            }, center_z=center_z, pretrain_model=pretrain_model_param_dict, return_grad=True, dynamic_partition=True, grad_norm_=args.grad_norm_)
    ...
    argmin_list = np.argmin(mini_grad_list_, 1)
    print("--")
    print("min(np.argmin(mini_grad_list_, 1)) ", min(np.argmin(mini_grad_list_, 1)))
    print("max(np.argmin(mini_grad_list_, 1)) ", max(np.argmin(mini_grad_list_, 1)))
    print("set(np.argmin(mini_grad_list_, 1)) ", set(np.argmin(mini_grad_list_, 1)))
...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None,
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        logits = model_obj({
            ...
            for key, value in batch_data.items()
            if key not in {consts.FIELD_USER_ID, consts.FIELD_LABEL}
        }, center_z=center_z, pretrain_model=pretrain_model_param_dict, grad_norm_=args.grad_norm_)
        prob = torch.sigmoid(logits).detach().cpu().numpy()
        y = batch_data[consts.FIELD_LABEL].view(-1, 1)
        # overall_auc, _, _, _ = new_metrics.calculate_overall_auc(prob, y)
        # ndcg = 0
