...
def parse_args():
    ...
    parser.add_argument("--test_file_overall", type=str, default="alipay", help="Bucket name for external storage")
...
def predict_dynamic(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None,
    ...
    for group_index in range(args.group_num):
        model_obj[group_index].eval()
        model_obj[group_index].to(device)
    # model_obj.eval()
    # model_obj.to(device)
    ...
    # user_id_list = []
    # center_z.to(device)
    center_z_list = center_z.to(device)
    # print("center_z.size() ", center_z.size())
    # center_z.cuda()
    predict_dataset_lens = 0
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        for group_index in range(args.group_num):
            model_obj[group_index].eval()
            # print(model_obj[group_index])
            center_z = center_z_list[group_index]
            logits, grad, mini_grad = model_obj[group_index]({
                key: value.to(device)
                for key, value in batch_data.items()
                if key not in {consts.FIELD_USER_ID, consts.FIELD_LABEL}
            }, center_z=center_z, pretrain_model=pretrain_model_param_dict, return_grad=True, dynamic_partition=True)
            # grad = grad.detach().cpu().numpy()
            # mini_grad = mini_grad.detach().cpu().numpy()
            prob = torch.sigmoid(logits).detach().cpu().numpy()
            # prob = torch.sigmoid(logits).detach().cpu()
            # y = batch_data[consts.FIELD_LABEL].view(-1, args.group_num)
            # y = batch_data[consts.FIELD_LABEL].view(-1, args.group_num)
            ...
            # grad_list.extend(grad)
            # grad_list.append(torch.from_numpy(np.array(grad).transpose((1, 0, 2))))
            # grad_list.append(torch.from_numpy(np.array(grad).transpose((1, 0, 2))))
            grad_list.append(grad)

            # mini_grad_list.extend(np.array(mini_grad).reshape(-1))
            # print(np.array(mini_grad).shape)
            # mini_grad_list.extend(np.array(mini_grad))
            # mini_grad_list.extend(np.array(mini_grad).transpose((1, 0)))
            # mini_grad_list.append(np.array(mini_grad))
            mini_grad_list.append(mini_grad)
            ...
            if group_index == 0:
                ...
                y_list_.extend(np.array(y))
                predict_dataset_lens += np.array(y).shape[0]
            ...
            #     )
            # )
            # buffer_list = [[str(user_id), float(score), float(label)]
            buffer_list = [[str(user_id.numpy()), float(score), float(label)]
                # torch.Tensor([int(user_id), float(score), float(label)])
                for user_id, score, label
                in zip(
                    batch_data[consts.FIELD_USER_ID],
                    prob,
            ...
            buffer.append(np.array(buffer_list))
            # buffer.append(
            #     np.array([str(user_id), float(score), float(label)])
            #     # torch.Tensor([int(user_id), float(score), float(label)])
            #     for user_id, score, label
            #     in zip(
            #         batch_data[consts.FIELD_USER_ID],
        ...
        # pred_list 2, 1024, 2
        # grad_list 2, 1024, 96
        # mini_grad_list 2, 1024, 2
        # buffer 2, 1024, 2, 3
        # print("=-=-")
        # print(torch.stack(pred_list).transpose(0, 1).squeeze(-1).size())
        # print(torch.Tensor(buffer).size())
        # print(torch.stack(buffer).size())
        # print(torch.Tensor(buffer).transpose(0, 1).size())
        # print(torch.stack(buffer).transpose(0, 1).size())
        # print(torch.stack(grad_list).transpose(0, 1).size())
        # print(torch.stack(grad_list).size())
        # print(torch.stack(mini_grad_list).transpose((0, 1)).size())
        print("++++")
        print(np.array(pred_list).shape)
        print(np.array(pred_list).swapaxes(0, 1).shape)
        print(np.array(grad_list).shape)
        print(np.array(grad_list).swapaxes(0, 1).shape)
        print(np.array(mini_grad_list).shape)
        print(np.array(mini_grad_list).swapaxes(0, 1).shape)
        print(np.array(buffer).shape)
        print(np.array(buffer).swapaxes(0, 1).shape)
        # pred_list_.extend(np.array(pred_list).transpose((0, 1)))
        pred_list_.extend(np.array(pred_list).swapaxes(0, 1))
        # pred_list_.extend(torch.stack(pred_list).transpose(0, 1))
        # y_list_.append(y_list)
        # buffer_.extend(torch.stack(buffer).transpose(0, 1))
        # buffer_.extend(torch.stack(buffer).transpose(0, 1))
        # buffer_.extend(np.array(buffer).transpose((0, 1)))
        buffer_.extend(np.array(buffer).swapaxes(0, 1))
        # grad_list_.extend(torch.stack(grad_list).transpose(0, 1))
        # grad_list_.extend(np.array(grad_list).transpose((0, 1)))
        grad_list_.extend(np.array(grad_list).swapaxes(0, 1))
        # mini_grad_list_.extend(mini_grad_list)
        # mini_grad_list_.append(mini_grad_list)
        # mini_grad_list_.extend(np.array(mini_grad_list).transpose((1, 0)))
        # mini_grad_list_.extend(torch.stack(mini_grad_list).transpose((0, 1)))
        # mini_grad_list_.extend(np.array(mini_grad_list).transpose((0, 1)))
        mini_grad_list_.extend(np.array(mini_grad_list).swapaxes(0, 1))

    # pred_list_ = torch.stack(pred_list_)
    # pred_list_ = np.array(pred_list_)

    # for grad in grad_list_:
    #     print(grad.shape)
    # grad_list_ = np.array(torch.cat(grad_list_, dim=0))
        ...
        # buffer 2, 1024, 2, 3
    print("&&&&")
    print(np.array(pred_list_).shape)
    print(np.array(grad_list_).shape)
    print(np.array(mini_grad_list_).shape)
    print(np.array(buffer_).shape)
    pred_list_ = np.array(pred_list_)
    mini_grad_list_ = np.array(mini_grad_list_)
    y_list_ = np.array(y_list_)
    buffer_ = np.array(buffer_)

    print("-" * 50)
    print(pred_list_.shape)
    # print(grad_list_.shape)
    print(mini_grad_list_.shape)
    # print(y_list_.shape)
    print(buffer_.shape)
    # print(np.argmin(mini_grad_list_).shape)
    # print(np.argmin(mini_grad_list_, 0).shape)
    print(np.argmin(mini_grad_list_, 1).shape)
    print(np.argmin(mini_grad_list_, 1).reshape(-1).shape)
    # pred_list = pred_list_[np.argmin(mini_grad_list_, 1).reshape(-1)]
    print("*" * 30)
    print(predict_dataset_lens)
    print(np.array(pred_list_).reshape(predict_dataset_lens, -1).shape)
    print(np.array(buffer_).reshape(predict_dataset_lens, -1).shape)
    print(np.argmin(mini_grad_list_, 1))
    # pred_list = np.array(pred_list_).reshape(predict_dataset_lens, -1)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    pred_list = np.array(pred_list_).reshape(predict_dataset_lens, -1)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    y_list = y_list_
    # buffer = buffer_[np.argmin(mini_grad_list_, 1).reshape(-1)]

    # buffer = np.array(buffer_).reshape(predict_dataset_lens, -1)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    # buffer = np.array(buffer_)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1), range(len(np.argmin(mini_grad_list_, 1)))]
    buffer = np.array(buffer_, dtype=np.float)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    # buffer = np.array(buffer_, dtype="int, float, float")[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    # buffer = np.array(buffer_, dtype=float)[range(len(np.argmin(mini_grad_list_, 1))), np.argmin(mini_grad_list_, 1)]
    # buffer = np.array(buffer, dtype='2str, 3int32, int32')
    # buffer = np.array(buffer, dtype='int, float, float')
    # buffer[:, 0] = buffer[:, 0].astype("str")
    # buffer[:, 1] = buffer[:, 1].astype("float")
    # buffer[:, 2] = buffer[:, 2].astype("float")
    # buffer[:, 0] = buffer[:, 0].astype(np.str)
    # buffer[:, 1] = buffer[:, 1].astype(np.float)
    # buffer[:, 2] = buffer[:, 2].astype(np.float)

    # buffer__ = []
    # for index, line in enumerate(buffer):
    #     user_id, score, label = line
    #     print()
    ...
    #     print(type(score))
    #     print(type(label))
    #     buffer__.append([str(user_id), float(score), float(label)])
    #     print([str(user_id), float(score), float(label)])
    #     # buffer[index] =
    # # buffer = np.array(buffer__)
    # buffer = buffer__
    
    # buffer__ = [[str(user_id.numpy()), float(score), float(label)]
    #           for (user_id, score, label)
    #           in buffer]
    # buffer = buffer__
    ...
    # pred_list = pred_list_[np.argmin(mini_grad_list_, 1)]
    # buffer = buffer_[np.argmin(mini_grad_list_, 1)]
    # print("-" * 50)
    # print(pred_list.shape)
    # print(buffer.shape)
    ...
    # return pred_list.tolist(), y_list.tolist(), buffer.tolist()
...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None, pretrain_model_param_dict=None,
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        logits = model_obj({
            ...
            for key, value in batch_data.items()
            if key not in {consts.FIELD_USER_ID, consts.FIELD_LABEL}
        }, center_z=center_z, pretrain_model=pretrain_model_param_dict)
        prob = torch.sigmoid(logits).detach().cpu().numpy()
        y = batch_data[consts.FIELD_LABEL].view(-1, 1)
        # overall_auc, _, _, _ = new_metrics.calculate_overall_auc(prob, y)
        # ndcg = 0
...
def train(train_dataloader_list, predict_dataloader_list, base_model_obj, meta_model_obj, device, args, pretrain_model_param_dict=None,
          train_dataset_length_list=None, predict_dataset_length_list=None, cluster_center_arr=None, predict_dataloader_overall=None):
    ...
    #     pretrain_model_param_dpict = ckpt.state_dict()
    ...
    for group_index in range(args.group_num):
        ...
        best_auc_ckpt_path_dict[group_index] = os.path.join(args.checkpoint_dir,
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        #     center_z = torch.Tensor(cluster_center_arr[group_index])
        ...
        y_list_overall = []
        buffer_overall = []
        pred_list_overall, y_list_overall, buffer_overall = predict_dynamic(
            predict_dataset=predict_dataloader_overall,
            ...
            pred_list_overall=pred_list_overall,
            buffer_overall=buffer_overall,
            center_z=torch.Tensor(cluster_center_arr),
            # predict_dataloader_overall=predict_dataloader_overall
        )
...
def main_worker(_):
    ...
    print("cluster_center_list.shape ", cluster_center_list.shape)
    # cluster_center_list = {}
    ...
    test_file_overall = args.test_file_overall
    for group_index in range(args.group_num):
    ...
    predict_dataset_overall = meta_sequence_dataloader.MetaSequenceDataLoader(
        # table_name=args.tables.split(',')[group_index],
        # table_name=test_files.split(',')[group_index],
        # table_name=test_files[group_index],
        table_name=test_file_overall,
        slice_id=0,
        slice_count=args.num_loading_workers,
        is_train=False,
    )
    predict_dataloader_overall = torch.utils.data.DataLoader(
        predict_dataset_overall,
        batch_size=args.batch_size,
        num_workers=args.num_loading_workers,
        pin_memory=True,
        collate_fn=predict_dataset_overall.batchify,
        drop_last=False
    )
