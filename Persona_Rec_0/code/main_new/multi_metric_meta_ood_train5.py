...
def parse_args():
    ...
    parser.add_argument("--max_epoch", type=int,  default=10, help="Max epoch")
    parser.add_argument("--max_epoch_ood", type=int,  default=10, help="Max epoch")
...
def predict_stage1(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None):
...
def train_stage1(train_dataset, model_obj, device, args, pred_dataloader, best_model=None):
    ...
    # print(all_layers_list)
    print(args.stage1_layer_list)
    # print(list(set(all_layers_list) - set(args.stage1_layer_list)))
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        if pred_overall_auc > best_auc:
            ...
            best_model = deepcopy(model_obj)
    return best_model


# def train(train_dataset, model_obj, device, args, bucket, pred_dataloader):
def train_stage2(train_dataset, model_obj, device, args, pred_dataloader, model_load_name_set=None, best_model=None):
    if best_model is not None:
        # model_uncertainty_obj = deepcopy(best_model)
        model_obj = best_model
    criterion = nn.BCEWithLogitsLoss()
    ...
    best_ckpt_path = os.path.join(args.checkpoint_dir, "stage2_best_auc" + "_ood.pkl")
    # zero_grad_dict = {}
    # for name, parms in model_obj.named_parameters():
    #     zero_grad_dict[name] = torch.zeros_like(parms).to(device)
    train_threshold = 0.5
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for step, batch_data in enumerate(train_dataset, 1):
            ...
            }, train_ood_threshold=args.train_ood_threshold, stage=2)
        ...
        _threshold, _ood_threshold = predict_stage2(
            predict_dataset=pred_dataloader,
            model_obj=model_obj,
            device=device,
            args=args,
