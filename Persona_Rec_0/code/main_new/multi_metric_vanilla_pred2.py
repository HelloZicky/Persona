...
def predict(predict_dataset, model_obj, device, args, train_epoch, train_step, writer=None):
    ...
    for step, batch_data in enumerate(predict_dataset, 1):
        ...
        # user_ndcg = new_metrics.calculate_user_ndcg(5, prob, y)
        # user_ndcg, user_hr = new_metrics.calculate_user_ndcg_hr(user_id_list, prob, y)
    ...
    # if len(buffer) >= 0:
    #     writer.write(buffer, [0, 1, 2])
    # fpr, tpr, thresholds = new_metrics.roc_curve(np.array(y_list), np.array(pred_list), pos_label=1)
    # overall_auc = float(new_metrics.overall_auc(fpr, tpr))
    # print("-" * 50)
    # print("buffer")
    # print(buffer)
...
def train(train_dataset, model_obj, device, args, predict_dataloader_list):
    ...
    for group_index in range(args.group_num):
        ...
        #     # writer=writer
        # )
        pred_list, y_list, buffer = predict(
            predict_dataset=pred_dataloader,
            model_obj=model_obj,
            device=device,
            args=args,
