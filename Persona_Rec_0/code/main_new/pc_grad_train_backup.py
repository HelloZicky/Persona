...
def predict(predict_dataloader_list, model_obj, device, args, train_epoch, train_step, writer=None):
...
def train(train_dataset_list, predict_dataloader_list, model_obj, device, args):
...
def main_worker(_):
    ...
    # dataset_length_dict = {}
    dataset_length_list = []
    train_files, test_files = args.tables.split(';')
    ...
    for group_index, (dataset_length, train_dataset) in enumerate(zip(train_dataset_length_list, train_dataset_list)):
    ...
    # Setup training
    train(
        train_dataset_list=train_dataloader_list,
        predict_dataloader_list=predict_dataloader_list,
        model_obj=model_obj,
        device=device,
        args=args,
