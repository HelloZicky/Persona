...
class LoaderBase(torch.utils.data.IterableDataset):
    ...
    # def _reopen_reader(self, original_reader, slice_id):
    ...
    def __iter__(self):
        ...
        # slice_id = self._slice_id_init + worker_id
        # reader = self._reopen_reader(None, slice_id)

        # reader = self._reopen_reader(None)

        # logger.info("====== slice_id : %d ===== " % slice_id)
        # sys.stdout.flush()
