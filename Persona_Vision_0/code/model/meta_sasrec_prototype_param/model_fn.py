...
@model("meta_sasrec_prototype_param", MetaType.ModelBuilder)
class SasRec(nn.Module):
    def __init__(self, model_conf):
        ...
        self._meta_classifier_param_list = common.HyperNetwork_FC_param(
            # model_conf.id_dimension * 2,
            model_conf.id_dimension,
            # model_conf.classifier + [1],
            [model_conf.id_dimension] * model_conf.mlp_layers,
    ...
    def forward(self, features, fig1=False, pretrain_model=None):
        # Encode target item
        ...
        user_embedding = self._meta_classifier_param_list(user_state, hist_embed,
                                         user_state.size()[0],
                                         seq_length, pretrain_model=pretrain_model)
        if fig1:
            user_embedding1 = self._meta_classifier_param_list(user_state, trigger_embed,
                                                              user_state.size()[0],
                                                              trigger_seq_length, pretrain_model=pretrain_model)
